#' @title fleet_swordfishTunasHarp
#' @description This is a wrapper function that facilitates extracting information for the large pelagics harpoon fleet.
...
#' @examples \dontrun{
#' db <- fleet_swordfishTunasHarp(useLocal = F,
#'                     year = 2018,
...
#'                     usepkg = "roracle"
#'                     )
#' local <- fleet_swordfishTunasHarp(year = 2018,
#'                        useLocal = T,
...
#' \itemize{
#'   \item \code{marfSpp} = c(251,259)
#'   \item \code{isdbSpp} = c(72, 73)
#'   \item \code{tripcd_id} = c(72,73)
#' }
#' Licence Information for this fleet is accessible via the following calls:
#' \itemize{
#'   \item \code{Licence Type, Subtype, Gear and Species Information (if applicable)} \code{Mar.fleets::licCore[Mar.fleets::licCore$FLEET=="SWORDFISHTUNASHARP",]}
#'   \item \code{Licence Areas (if applicable)} \code{Mar.fleets::licAreas[Mar.fleets::licAreas$FLEET=="SWORDFISHTUNASHARP",]}
#'
...
#' }
#' @export
fleet_swordfishTunasHarp <- function(useLocal = NULL, ...){
  if(!paramOK(useLocal = useLocal, p=list(...))) stop("Please provide additional parameters as directed above")

    data = fleet_(fleet = "SWORDFISHTUNASHARP", marfSpp = c(251,259), isdbSpp = c(72, 73), area = "ALL", tripcd_id = c(72,73), useLocal = useLocal,...)
  return(data)
}
