...
#' \itemize{
#'   \item \code{Licence Type, Subtype, Gear and Species Information (if applicable)} \code{Mar.fleets::licCore[Mar.fleets::licCore$FLEET=="LPELAGICS",]}
#'   \item \code{Licence Areas (if applicable)} \code{Mar.fleets::licAreas[Mar.fleets::licAreas$FLEET=="LPELAGICS",]}
#'
...
fleet_hagfish <- function(useLocal = NULL, ...){
  if(!paramOK(useLocal = useLocal, p=list(...))) stop("Please provide additional parameters as directed above")
    data = fleet_(fleet = "HAGFISH", marfSpp = c(197), isdbSpp = c(241), gearPSOveride = 61, area = "ALL", tripcd_id = c(7063), useLocal = useLocal,...)
  return(data)
}
