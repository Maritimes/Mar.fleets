...
#' @param marfGear default is \code{c(61,62)}. This is a vector of MARFIS gear codes
...
#' \itemize{
#'   \item \code{marfSpp} = 197
#'   \item \code{isdbSpp} = 241
#'   \item \code{tripcd_id} = 7063
...
#' @export
fleet_hagfish <- function(marfGear = c(61,62), useLocal = NULL, socks = FALSE, ...){
  if (!socks) isDraft()
  if(!paramOK(useLocal = useLocal, p=list(...))) stop("Please provide additional parameters as directed above")
  valuesOK(valSent = marfGear, valID = "marfGear", valOK =   c(61,62))
  data = fleet_(fleet = "HAGFISH", marfSpp = 197, marfGear = marfGear, isdbSpp = 241, tripcd_id = 7063, useLocal = useLocal,...)
  return(data)
}
