...
#' @export
fleet_rockCrab <- function(marfGear = c(62), useLocal = NULL, ...){
  if(!paramOK(useLocal = useLocal, p=list(...))) stop("Please provide additional parameters as directed above")
  stop("wrapper in progress")
  data = fleet_(fleet = "ROCKCRAB", marfSpp = c(704,742), marfGear = marfGear, isdbSpp = c(2513),tripcd_id = c(2509), useLocal = useLocal,...)
  return(data)
}
