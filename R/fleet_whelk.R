...
#' \itemize{
#'   \item \code{Licence Type, Subtype, Gear and Species Information (if applicable)} \code{Mar.fleets::licCore[Mar.fleets::licCore$FLEET=="WHELK",]}
#'   \item \code{Licence Areas (if applicable)} \code{Mar.fleets::licAreas[Mar.fleets::licAreas$FLEET=="WHELK",]}
#'
...
fleet_whelk <- function(useLocal = NULL, ...){
  if(!paramOK(useLocal = useLocal, p=list(...))) stop("Please provide additional parameters as directed above")
    ...
    data = fleet_(fleet = "WHELK", marfSpp = c(615), isdbSpp = c(4210,4211), area = "ALL", tripcd_id = c(4211), useLocal = useLocal,...)
  return(data)
}
