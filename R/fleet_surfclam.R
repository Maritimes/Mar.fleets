...
#' }
#' @export
fleet_surfclam <- function(fleetOnly=T, ...){

  # set hardcoded -------------------------------------------------------------------------------------------------------------------------------------------
  lics <- data.frame(rbind(c(10,15,608)), stringsAsFactors=FALSE)
...
# Do stuff, and add to results ----------------------------------------------------------------------------------------------------------------------------
  ...
  if (length(isdb)>1 && class(isdb$ALL_ISDB_TRIPS)=="data.frame"){
    ...
    browser()
    bycatch <- do.call(get_bycatch, list(isTrips = unique(isdb$ALL_ISDB_TRIPS[!is.na(isdb$ALL_ISDB_TRIPS$TRIP_ID_MARF), "TRIP_ID_ISDB"]), marfSpID = args$marfSpp, args=args))
