    ...
    if(exists("debugTrips")) {
      ...
      if(args$useLocal){
        Mar.utils::get_data_tables(schema = "MARFISSCI", data.dir = args$data.dir,
                                   tables = c("PRO_SPC_INFO","NAFO_UNIT_AREAS","MON_DOCS"), quietly = TRUE, env = environment())
...
debugTripsMARFIS = args$cxn$thecmd(args$cxn$channel, debugQry)
      ...
      debugTripsMARFIS <- reshape2::melt(unique(debugTripsMARFIS[,c("TRIP_ID","LICENCE_ID", "VR_NUMBER_FISHING", "VR_NUMBER_LANDING","NAFO_AREAS", "GEAR_CODE", "DATE_FISHED","LANDED_DATE", "MON_DOC_ID", "MD_CODE")]), id.vars = c("TRIP_ID", "LICENCE_ID","NAFO_AREAS", "GEAR_CODE", "DATE_FISHED","LANDED_DATE", "MON_DOC_ID", "MD_CODE"))
      ...
      debugTripsMARFIS[which(debugTripsMARFIS$DATE_MIN <= as.Date(args$dateEnd) & debugTripsMARFIS$DATE_MAX >= as.Date(args$dateStart)),"MARFIS_DATERANGE"]<-TRUE
      ...
      debugTripsMARFIS<- debugTripsMARFIS[,c("TRIP_ISDB", "TRIP_MARFIS", "DATE_MIN", "DATE_MAX", "MARFIS_DATERANGE", "LICENCE_ID", "VR_NUMBER", "MARFIS_VESS_LIC_COMBO",
